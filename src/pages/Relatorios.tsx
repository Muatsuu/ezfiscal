...
import { FileDown, BarChart3, TrendingUp, Settings2, PlusCircle, PieChart as PieChartIcon } from "lucide-react";
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <Dialog open={showExportDialog} onOpenChange={setShowExportDialog}>
          <DialogTrigger asChild>
            <button className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-success/10 text-success text-xs font-semibold hover:bg-success/20 transition-colors border border-success/20">
              <Settings2 className="w-4 h-4" />
              Exportar
            </button>
          </DialogTrigger>
          <DialogContent className="max-w-sm">
            <DialogHeader><DialogTitle className="text-base">Exportar Relatório</DialogTitle></DialogHeader>
        ...
        {periodFilters.map((f) => (
          <button key={f.value} onClick={() => setPeriod(f.value)}
            className={`px-4 py-2 rounded-full text-xs font-medium whitespace-nowrap transition-all ${
              period === f.value ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:text-foreground"
            }`}>
            {f.label}
          </button>
        ))}
          ...
          {stats.comparativoMensal.length > 0 ? (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={stats.comparativoMensal} margin={{ left: 0, right: 10 }}>
                ...
                <Bar dataKey="valor" radius={[6, 6, 0, 0]} barSize={32} fill="url(#barGradient)" />
              </BarChart>
            </ResponsiveContainer>
          ) : (
        ...
        {stats.volumeMensal.length > 0 ? (
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={stats.volumeMensal} margin={{ left: 0, right: 10 }}>
              ...
              <Tooltip content={<CustomTooltip />} />
              <Area type="monotone" dataKey="count" stroke="hsl(210, 100%, 60%)" strokeWidth={2.5} fill="url(#colorVolume)"
